    ...
    <>
      ...
      {rows.map((row, index) => (
        <div key={index} className="mb-2 border" style={{ display: 'grid', gridTemplateColumns: '2.5fr 1fr 4fr 2fr 2fr', gap: '4px' }}>
          ...
          <textarea id={`numberOfPackages-${index}`} className="form-control h-100 resize-none" value={row.packages} onChange={e => { const nr=[...rows]; nr[index].packages=e.target.value; setRows(nr);} } placeholder="Number of packages" />
